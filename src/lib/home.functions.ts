...
export const toggleLikeNewsItem = createServerFn({ method: "POST" })
  .inputValidator((d: unknown) => {
    return (d as { id: string; action: string }).id
      ? { id: (d as { id: string; action: string }).id, action: (d as { action: string }).action }
      : { id: "", action: "like" };
  })
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    ...
    await supabaseAdmin
      .from("news" as any)
      .update({ upvotes: Math.max(0, ((news as any).upvotes || 0) + change) })
      .eq("id", data.id);
    return { ok: true };
  });
...
export const getNewsComments = createServerFn({ method: "GET" })
  .inputValidator((d: unknown) => {
    return (d as { newsId: string }).newsId
      ? { newsId: (d as { newsId: string }).newsId }
      : { newsId: "" };
  })
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const addNewsComment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (news) {
      await supabaseAdmin
        .from("news" as any)
        .update({ comment_count: ((news as any).comment_count || 0) + 1 })
        .eq("id", data.newsId);
    }
...
export const deleteNewsComment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (news) {
      await supabaseAdmin
        .from("news" as any)
        .update({ comment_count: Math.max(0, ((news as any).comment_count || 0) - 1) })
        .eq("id", data.newsId);
    }
