...
function IncidentsAdmin() {
  ...
  const colleges = useQuery({
    queryKey: ["col-map"],
    queryFn: async () => (await supabase.from("colleges").select("id, name")).data ?? [],
  });
  const q = useQuery({
    queryKey: ["admin-incidents"],
    queryFn: async () =>
      (await supabase.from("incidents").select("*").order("first_seen", { ascending: false }))
        .data ?? [],
  });
  ...
  return (
    <div>
      ...
      <Select value={status} onValueChange={setStatus}>
        <SelectTrigger className="mt-4 w-48 bg-surface">
          <SelectValue />
        </SelectTrigger>
        <SelectContent>
          {["All", "active", "investigating", "resolved", "dismissed"].map((s) => (
              ...
              <tr key={i.id} className="border-t border-border">
                ...
                <td className="p-3">
                  <Select
                    ...
                    onValueChange={async (v) => {
                      await upd({ data: { token: token!, id: i.id, patch: { status: v } } });
                      toast.success("Status updated");
                      q.refetch();
                    }}
                  >
                    <SelectTrigger
                      className={cn("h-7 w-36 border text-xs", statusColor(i.status ?? "active"))}
                    >
                      <SelectValue />
                    </SelectTrigger>
                    <SelectContent>
                      {["active", "investigating", "resolved", "dismissed"].map((s) => (
                        <SelectItem key={s} value={s}>
                          {s}
                        </SelectItem>
                      ))}
                ...
                <td className="p-3">
                  <Button
                    ...
                    onClick={async () => {
                      await del({ data: { token: token!, ids: [i.id] } });
