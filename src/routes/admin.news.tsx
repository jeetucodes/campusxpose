      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <div className="space-y-2">
                ...
                {imageUrl && (
                  <img
                    src={imageUrl}
                    alt="Preview"
                    className="mt-2 h-20 w-auto object-cover rounded-md border-2 border-border"
                  />
                )}
              </div>
              <Button
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          {isLoading ? (
            ...
            <p className="text-muted-foreground border-2 border-dashed border-border p-8 text-center rounded-xl">
              No news items found.
            </p>
          ) : (
            news.map((item: any) => (
              <Card
                key={item.id}
                className={`p-4 border-2 border-border shadow-[4px_4px_0px_0px_rgba(0,0,0,1)] rounded-xl flex flex-col sm:flex-row gap-4 ${!item.is_active ? "opacity-50" : ""}`}
              >
                {item.image_url && (
                  <img
                    src={item.image_url}
                  ...
                  <div className="flex items-center gap-2">
                    <Switch
                      ...
                      onCheckedChange={(checked) =>
                        toggleStatus.mutate({ id: item.id, is_active: checked })
                      }
                    />
