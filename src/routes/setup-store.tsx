...
import { runStoreMigration } from "@/lib/setup-store.functions";
import { Button } from "@/components/ui/button";
import { cn } from "@/lib/utils";

/**
 * ONE-TIME SETUP PAGE for Store tables.
 * Visit /setup-store once, then this page can be ignored.
 */
export const Route = createFileRoute("/setup-store")({
  head: () => ({
    meta: [{ title: "Store DB Setup — CampusXpose" }, { name: "robots", content: "noindex" }],
  }),
  component: SetupPage,
});
...
function SetupPage() {
  ...
  const migrate = useServerFn(runStoreMigration);
  ...
  const allVerified =
    result &&
    ["store_categories", "store_banners", "store_products"].every((t) => result.verify[t] === true);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-paper p-4">
      ...
      >
        <h1 className="mb-2 font-display text-2xl font-black">🛠️ Store DB Setup</h1>
        <p className="mb-6 text-sm text-muted-foreground">
          This creates the 3 tables needed for the dynamic Shop feature:{" "}
          <code className="rounded bg-muted px-1 text-xs">store_categories</code>,{" "}
          <code className="rounded bg-muted px-1 text-xs">store_banners</code>,{" "}
          <code className="rounded bg-muted px-1 text-xs">store_products</code>
        </p>
          ...
          <div className="space-y-4">
            ...
            {allVerified ? (
              <div className="rounded-xl border-2 border-green-400 bg-green-50 p-4 text-center">
                ...
                <p className="mt-1 text-sm text-green-700">Store database is now ready to use.</p>
                <Button className="mt-3" onClick={() => navigate({ to: "/shop" })}>
                  Go to /shop →
                </Button>
              </div>
            ) : (
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-sm text-amber-700">
                      The Management API requires a Personal Access Token in Vercel. 
