...
function NewsPage() {
  ...
  const toggleLike = useMutation({
    mutationFn: ({ id, action }: { id: string; action: "like" | "unlike" }) =>
      toggleLikeNewsItem({ data: { id, action } }),
      ...
      queryClient.setQueryData(["home"], (old: any) => {
        ...
        return {
          ...
          news: old.news.map((n: any) =>
            n.id === id
              ? { ...n, upvotes: Math.max(0, (n.upvotes || 0) + (action === "like" ? 1 : -1)) }
              : n,
          ),
        };
      });
      ...
      <div className="mx-auto max-w-7xl px-4 py-8 md:py-12">
        <div className="mb-8 flex items-center gap-4">
          <Button
            ...
            variant="outline"
            size="icon"
            className="h-10 w-10 border-2 border-border bg-white hover:bg-accent/10 transition-colors"
        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-6">
          {news.length === 0 ? (
            <div
              className="break-inside-avoid mb-6 text-center py-12 text-muted-foreground border-2 border-border bg-white"
              style={{ borderRadius: WOBBLY_MD }}
            >
    ...
    >
      <div className="p-5 flex flex-col h-full">
        ...
        {item.text && item.text.trim() !== "" && (
          <p className="font-medium text-foreground text-base leading-relaxed flex-1 mb-2">
            {item.text}
          </p>
        )}
        {item.link_url && (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="inline-flex items-center gap-1.5 text-primary text-sm font-bold mt-2 underline-offset-4 hover:underline hover:text-accent w-fit"
          >
      ...
      <AnimatePresence>{showComments && <NewsComments newsId={item.id} />}</AnimatePresence>
    </motion.div>
  );
}
...
function NewsComments({ newsId }: { newsId: string }) {
    ...
    mutationFn: () =>
      addNewsComment({
        data: {
          ...
          content,
          hashedId: identity.hashedId || undefined,
        },
      }),
    onSuccess: (newComment) => {
      setContent("");
      queryClient.setQueryData(["news-comments", newsId], (old: any) => [
        ...(old || []),
        newComment,
      ]);
      ...
      queryClient.setQueryData(["home"], (old: any) => {
        ...
        return {
          ...old,
          news: old.news.map((n: any) =>
            n.id === newsId ? { ...n, comment_count: (n.comment_count || 0) + 1 } : n,
          ),
        };
      });
  ...
  const deleteComment = useMutation({
    mutationFn: (commentId: string) =>
      deleteNewsComment({ data: { commentId, newsId, hashedId: identity.hashedId! } }),
    onSuccess: (_, commentId) => {
      ...
      queryClient.setQueryData(["home"], (old: any) => {
        ...
        return {
          ...old,
          news: old.news.map((n: any) =>
            n.id === newsId ? { ...n, comment_count: Math.max(0, (n.comment_count || 0) - 1) } : n,
          ),
        };
      });
        ...
        {isLoading ? (
          ...
          <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
            {comments.map((c: any) => (
              <div
                key={c.id}
                className="bg-white border-2 border-border p-3 group relative"
                style={{ borderRadius: WOBBLY_MD }}
              >
