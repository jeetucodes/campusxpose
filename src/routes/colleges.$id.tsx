...
export const Route = createFileRoute("/colleges/$id")({
  ...
  head: ({ params, loaderData }) => {
    ...
    const description = c?.name
      ? `Anonymous student reviews, ratings and incident reports for ${c.name}${location ? `, ${location}` : ""}. Read the real campus story.`.slice(
          0,
          160,
        )
...
function CollegeDetail() {
  ...
  const ratingsQ = useQuery({
    queryKey: ["ratings", id],
    queryFn: async () =>
      (await supabase.from("ratings").select("*").eq("college_id", id)).data ?? [],
  });
  ...
  useEffect(() => {
    const ch = supabase
      ...
      .on(
        "postgres_changes",
        { event: "INSERT", schema: "public", table: "posts", filter: `college_id=eq.${id}` },
        () => {
          postsQ.refetch();
        },
      )
    ...
    return (
      <SiteShell hideFooter>
        <div className="mx-auto max-w-4xl space-y-4 px-4 py-10">
          <Skeleton className="h-40 rounded-xl bg-surface" />
          <Skeleton className="h-60 rounded-xl bg-surface" />
        </div>
      </SiteShell>
    );
  }
  ...
  const avg = (k: string) =>
    ratings.length
      ? ratings.reduce((s, r) => s + ((r as any)[k] ?? 0), 0) / ratings.length
      : (c.total_rating ?? 0);
  ...
  return (
    <SiteShell hideFooter>
      <div className="mx-auto max-w-4xl px-4 py-8">
        {/* Header */}
        <div
          className="border-2 border-ink bg-white p-6 shadow-ink-soft"
        ...
        >
          <div className="flex flex-col md:flex-row md:items-start justify-between gap-6">
            <div className="flex-1">
              ...
              <div className="mt-3 flex flex-wrap items-center gap-2 text-sm font-semibold text-muted-foreground">
                <span className="flex items-center gap-1 bg-surface px-2 py-1 rounded-md border border-ink/10">
                  <MapPin className="h-3 w-3" /> {c.city}, {c.state}
                </span>
                <span className="bg-surface px-2 py-1 rounded-md border border-ink/10">
                  {Array.isArray((c as any).types) && (c as any).types.length
                    ? (c as any).types.join(", ")
                    : [c.type].join(", ")}
                </span>
                <span className="bg-surface px-2 py-1 rounded-md border border-ink/10">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {(c as any).website && (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="border-2 border-ink shadow-ink-soft font-bold rounded-lg hover:-translate-y-0.5 transition-transform"
                  >
                    <a
                      href={
                        (c as any).website.startsWith("http")
                          ? (c as any).website
                          : `https://${(c as any).website}`
                      }
                      target="_blank"
                      rel="noreferrer"
                    >
            ...
            <div className="flex flex-col items-center md:items-end w-full md:w-auto shrink-0 bg-surface-2 p-4 rounded-xl border-2 border-ink shadow-ink-soft text-center min-w-[120px]">
              <div className="text-xs font-black uppercase tracking-widest text-ink/60 mb-1">
                Overall
              </div>
              <div
                className={cn(
                  "font-display text-5xl font-black",
                  ratingColor(Number(c.total_rating ?? 0)),
                )}
              >
          ...
          <div className="mt-6 flex flex-col md:flex-row gap-3">
            <Button
              asChild
              className="flex-1 bg-accent text-accent-foreground hover:bg-accent/90 shadow-ink-soft border-2 border-ink"
              style={{ borderRadius: "12px 4px 10px 4px" }}
            >
              <Link to="/report" search={{ college: id }}>
                <Zap className="mr-2 h-5 w-5 fill-current" /> Report Incident
              </Link>
            </Button>
            <Button
              asChild
              variant="outline"
              className="flex-1 border-2 border-ink hover:-rotate-1 transition-transform"
              style={{ borderRadius: "4px 12px 4px 10px" }}
            >
              <Link to="/community/$collegeId" params={{ collegeId: id }}>
                <MessageCircle className="mr-2 h-5 w-5" /> Campus Students Chats
              </Link>
            </Button>
          </div>
        </div>
        ...
        {/* Ratings breakdown */}
        <section
          className="mt-8 border-2 border-ink bg-white p-6 shadow-ink-soft"
          style={{ borderRadius: "6px 18px 6px 20px / 20px 6px 18px 6px" }}
        >
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            <Button
              size="sm"
              variant="outline"
              className="w-full sm:w-auto border-2 border-ink bg-postit text-ink shadow-ink-soft hover:-rotate-2 transition-transform"
              style={{ borderRadius: "12px 4px 10px 4px" }}
                ...
                <div key={cat.key}>
                  ...
                  <div className="h-2 overflow-hidden rounded-full bg-surface-2">
                    <motion.div
                      ...
                      viewport={{ once: true }}
                      className={cn("h-full rounded-full", ratingBarColor(v))}
                    />
                  </div>
                </div>
              );
        ...
        <section className="mt-8">
          <h2 className="mb-4 flex items-center gap-2 font-display text-xl font-bold">
            Incident Reports
            <span className="rounded-full bg-destructive/15 px-2 py-0.5 text-sm font-bold text-destructive shadow-sm">
              {incidents.length + posts.length}
            </span>
          </h2>
            ...
            {INCIDENT_CATEGORIES.map((cat) => {
              ...
              return (
                <div
                  ...
                  className={cn(
                    "sketch-card cursor-pointer p-4 text-left border-2 shadow-ink-soft transition-transform hover:-translate-y-1",
                    open ? "border-accent bg-accent/10" : "border-ink bg-white",
                  )}
                >
          ...
          <Dialog open={!!openCat} onOpenChange={(v) => !v && setOpenCat(null)}>
            <DialogContent
              className="max-h-[85vh] overflow-y-auto border-2 border-ink bg-white p-6 shadow-ink sm:max-w-[600px]"
              style={{ borderRadius: "16px 6px 18px 4px / 6px 16px 4px 18px" }}
            >
              <DialogHeader>
              ...
              <div className="mt-2 space-y-3">
                {openCat &&
                  incidents
                    .filter((i) => i.category === openCat)
                    .map((i) => (
                      <div
                        key={i.id}
                        className="border-2 border-ink bg-white p-4 shadow-ink-soft"
                        style={{ borderRadius: "12px 4px 10px 4px" }}
                      >
                        <div className="font-semibold">{i.title}</div>
                        <div className="mt-1 text-xs font-medium text-muted-foreground">
                          {i.affected_count} affected · severity {i.severity}
                        </div>
                      </div>
                    ))}
                {openCat &&
                  posts
                    .filter((p) => (p.category ?? "general") === openCat)
                    .map((p) => (
                      <div
                        key={p.id}
                        className="border-2 border-ink bg-white p-4 shadow-ink-soft"
                      ...
                      >
                        ...
                        <div className="mt-2 inline-flex items-center gap-1 text-xs font-semibold text-muted-foreground">
                          {p.username}
                          {p.username && verified.has(p.username) && (
                ...
                {openCat &&
                  incidents.filter((i) => i.category === openCat).length +
                    posts.filter((p) => (p.category ?? "general") === openCat).length ===
                    0 && (
        ...
        <section className="mt-8">
          ...
          <div className="space-y-4">
            {incidents.slice(0, 5).map((i) => (
              <div
                key={i.id}
                className="border-2 border-ink bg-white p-5 shadow-ink-soft"
                style={{ borderRadius: "10px 16px 14px 10px / 16px 10px 16px 14px" }}
              >
                <div className="flex flex-col sm:flex-row sm:items-center items-start justify-between gap-2 sm:gap-3">
                  <h3 className="font-semibold">{i.title}</h3>
                  <span
                    className={cn(
                      "shrink-0 rounded-full border-2 border-ink px-2 py-0.5 text-xs capitalize shadow-ink-soft",
                      statusColor(i.status ?? "active"),
                    )}
                  >
                    {i.status}
                  </span>
                ...
                <p className="mt-1 text-sm font-bold text-accent">
                  {i.affected_count} students reported the same issue
                </p>
                {i.ai_summary && (
                  <p className="mt-2 line-clamp-2 text-sm text-muted-foreground">{i.ai_summary}</p>
                )}
                {i.ai_verdict && (
                  <p className="mt-3 rounded-lg border border-dashed border-warning bg-warning/10 p-2 text-xs font-medium text-warning">
                    ⚖️ {i.ai_verdict}
                  </p>
                )}
                {(i.total_amount ?? 0) > 0 && (
                  <p className="mt-2 text-sm font-medium">
                    Total reported: {inr(i.total_amount ?? 0)}
                  </p>
                )}
              </div>
            ))}
          ...
          <div className="space-y-3">
            {posts.map((p) => (
              <PostCard
                key={p.id}
                post={p}
                userVote={myVotes[p.id] ?? null}
                onVoted={() => {
                  postsQ.refetch();
                  myVotesQ.refetch();
                }}
                evidence={evidence.filter((e) => e.post_id === p.id)}
              />
            ))}
            {posts.length === 0 && (
              <p className="text-sm text-muted-foreground">
                No posts yet. Be the first to share the truth.
              </p>
            )}
          </div>
        </section>
...
function Stat({ n, l }: { n: number; l: string }) {
  return (
    <div
      className="flex flex-col items-center justify-center border-2 border-dashed border-ink bg-accent/5 p-3"
...
function PostCard({
  ...
  const doVote = async (dir: "up" | "down") => {
    ...
    try {
      ...
      if (prevMyVotes)
        queryClient.setQueryData(["my-votes", post.college_id, hashedId], prevMyVotes);
    ...
    <div className="rounded-xl border border-border bg-surface p-4">
      <div className="flex items-center gap-2 text-xs text-muted-foreground">
        ...
        <span className="inline-flex items-center gap-1 font-medium text-foreground">
          {post.username}
          {post.username && verified.has(post.username) && <VerifiedBadge />}
        </span>
        <span>· {timeAgo(post.created_at)}</span>
        <span className="ml-auto rounded-full bg-surface-2 px-2 py-0.5 capitalize">
          ...
          {evidence.map((e) => (
            <a key={e.id} href={e.file_url} target="_blank" rel="noreferrer" className="block">
              {e.type === "image" ? (
                ...
                <div className="flex h-16 w-full items-center justify-center rounded-lg bg-surface-2 border border-border text-xs text-muted-foreground">
                  {e.type} file
                </div>
              )}
            </a>
      ...
      <div className="mt-3 flex items-center gap-2">
        <button
          ...
          className={cn(
            ...
            upActive ? "bg-success/15 text-success" : "bg-surface-2 hover:text-success",
          )}
        >
        ...
        <button
          ...
          className={cn(
            ...
            downActive
              ...
              : "bg-surface-2 hover:text-destructive",
          )}
        >
        ...
        <button
          ...
          className={cn(
            "inline-flex items-center gap-1 rounded-full px-2.5 py-1 text-xs",
            commentsOpen ? "bg-primary/15 text-primary" : "bg-surface-2 hover:text-primary",
          )}
        >
...
function RatingModal({
  ...
  const submit = async () => {
    if (!hashedId) return;
    if (Object.values(vals).some((v) => v < 1)) {
    ...
    try {
      const res = await rate({
        data: {
          ...
          infrastructure: vals.infrastructure,
          campusLife: vals.campusLife,
          value: vals.value,
        },
      });
    ...
    <>
      <Dialog open={open} onOpenChange={onOpenChange}>
        <DialogContent className="border-border bg-surface">
          <DialogHeader>
            <DialogTitle>Rate This College</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            {fields.map((f) => (
                  ...
                  {[1, 2, 3, 4, 5].map((i) => (
                    <button key={i} onClick={() => setVals((p) => ({ ...p, [f.key]: i }))}>
                      <Star
                        className={cn(
                          "h-6 w-6",
                          i <= vals[f.key] ? "fill-warning text-warning" : "text-border",
                        )}
                      />
                    </button>
                  ))}
            ...
            <Button disabled={busy} onClick={submit} className="w-full rounded-full">
              Submit Anonymously
            </Button>
          </div>
        </DialogContent>
      ...
      <AlertDialog open={alreadyRatedOpen} onOpenChange={setAlreadyRatedOpen}>
        <AlertDialogContent
          ...
          style={{ borderRadius: "16px 5px 14px 5px / 5px 14px 5px 16px" }}
        >
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-foreground/80 font-medium">
              You've already rated this college! We only allow one rating per user to keep things
              fair and accurate.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
