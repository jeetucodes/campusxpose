...
function CommunityAdmin() {
  ...
  const exportChat = () => {
    ...
    a.download = `chat_${collegeId}_${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
  };
    ...
    <div>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        <Select value={collegeId} onValueChange={setCollegeId}>
          <SelectTrigger className="w-64 bg-surface">
        ...
        {collegeId && (
          <>
            <Button variant="outline" className="rounded-full" onClick={exportChat}>
              📥 Export First
            </Button>
            <Button
              variant="destructive"
              ...
              onClick={async () => {
                if (!window.confirm(`Delete all ${msgs.length} messages?`)) return;
                const r = await clear({ data: { token: token!, collegeId } });
                toast.success(`Cleared ${r.deleted} messages`);
                setMsgs([]);
              }}
            >
      ...
      <div className="mt-4 space-y-2">
        {msgs.map((m) => (
          <div
            key={m.id}
            className={cn(
              "group flex items-center gap-2 rounded-lg bg-surface p-3 text-sm",
              m.is_incident_signal && "border-l-2 border-warning",
            )}
          >
            ...
            <button
              ...
              onClick={async () => {
                await delMsg({ data: { token: token!, id: m.id } });
