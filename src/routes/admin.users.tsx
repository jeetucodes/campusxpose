...
function UsersAdmin() {
  ...
  const doBan = async (u: any) => {
    await ban({
      data: { token: token!, userHash: u.hash, username: u.username, reason: "Admin action" },
    });
    toast.success("Shadow banned");
    q.refetch();
  };
  ...
  const doVerify = async (u: any) => {
    await setVerified({
      data: { token: token!, userHash: u.hash, username: u.username, verified: !u.verified },
    });
  ...
  const doRename = async (u: any) => {
    const next = window.prompt(
      `New username for ${u.username} (letters, numbers, _ only):`,
      u.username,
    );
    if (!next || next === u.username) return;
    const res = await rename({
      data: { token: token!, userHash: u.hash, oldUsername: u.username, newUsername: next.trim() },
    });
    if (!res.ok) {
  ...
  const doResetAvatar = async (u: any) => {
    await setAvatar({
      data: { token: token!, userHash: u.hash, username: u.username || undefined, url: null },
    });
  ...
  return (
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <div className="sketch-card wobbly-sm bg-surface px-4 py-2 text-sm flex items-center gap-2 font-bold">
            <span className="text-lg text-success">{realCount}</span>{" "}
            <span className="text-muted-foreground uppercase tracking-wider text-xs">Real</span>
          </div>
          <div className="sketch-card wobbly-sm bg-surface px-4 py-2 text-sm flex items-center gap-2 font-bold">
            <span className="text-lg text-warning">{forgotCount}</span>{" "}
            <span className="text-muted-foreground uppercase tracking-wider text-xs">
              Forgot Me
            </span>
          </div>
          <div className="sketch-card wobbly-sm bg-surface px-4 py-2 text-sm flex items-center gap-2 font-bold">
            <span className="text-lg text-accent">{verifiedCount}</span>{" "}
            <span className="text-muted-foreground uppercase tracking-wider text-xs">Verified</span>
          </div>
        </div>
      </div>
      ...
      <div className="mt-4 space-y-4 md:hidden">
              ...
              <div key={u.hash} className="sketch-card wobbly-md p-4 relative">
                <div className="flex items-center justify-between gap-2 pr-8">
                  <span className="flex items-center gap-2 font-bold text-lg font-display text-ink">
                    ...
                    {u.forgotten && (
                      <span className="border-2 border-warning shadow-ink-soft rounded px-1.5 py-0.5 text-[10px] font-bold text-warning uppercase">
                        Forgot
                      </span>
                    )}
                  </span>
                  <span className={cn("text-xs font-bold uppercase tracking-wider", r.cls)}>
                    {r.label}
                  </span>
                </div>
                <div className="mt-1 font-mono text-xs text-muted-foreground bg-surface-2 p-1.5 inline-block border-2 border-border wobbly-sm">
                  {u.hash.slice(0, 8)}
                </div>
                ...
                <div className="mt-4 flex gap-4 text-sm font-bold text-ink">
                  <span>
                    {u.posts} <span className="text-muted-foreground font-normal">posts</span>
                  </span>
                  <span>
                    {u.messages} <span className="text-muted-foreground font-normal">msgs</span>
                  </span>
                  <span>
                    {u.incidents} <span className="text-muted-foreground font-normal">inc.</span>
                  </span>
                </div>
        ...
        {q.isLoading && (
          <p className="text-sm font-medium font-display animate-pulse text-muted-foreground">
            Loading users…
          </p>
        )}
        {q.isError && (
          <p className="text-sm font-medium font-display text-destructive">
            Couldn't load users: {(q.error as Error)?.message ?? "Unknown error"}.{" "}
            <button className="underline font-bold" onClick={() => q.refetch()}>
              Retry
            </button>
          </p>
        )}
        {!q.isLoading && !q.isError && (q.data ?? []).length === 0 && (
          <p className="text-sm font-medium font-display text-muted-foreground">No users found.</p>
        )}
      </div>
      ...
      <div className="mt-4 hidden md:flex flex-col gap-3">
        ...
        {(q.data ?? [])
          ...
          .map((u) => {
            ...
            return (
              <div
                key={u.hash}
                className="sketch-card wobbly-sm grid grid-cols-[1.5fr_1fr_0.5fr_0.5fr_0.5fr_1fr_0.5fr_auto] items-center gap-4 px-6 py-4 hover:bg-surface-2 transition-colors"
              >
                <div className="flex items-center gap-2 font-display text-lg font-bold text-ink truncate">
                  ...
                  {u.forgotten && (
                    <span className="border-2 border-warning shadow-ink-soft rounded px-1.5 py-0.5 text-[10px] font-bold text-warning uppercase shrink-0">
                      Forgot
                    </span>
                  )}
                </div>
                <div className="font-mono text-xs text-muted-foreground bg-surface-2 p-1 border-2 border-border wobbly-sm truncate">
                  {u.hash.slice(0, 12)}
                </div>
                ...
                <div className={cn("text-xs font-bold uppercase tracking-wider", r.cls)}>
                  {r.label}
                </div>
                <div>
                  <UserActionsMenu
        ...
        {q.isError && (
          <p className="p-3 text-sm font-medium font-display text-destructive">
            Couldn't load users: {(q.error as Error)?.message ?? "Unknown error"}.{" "}
            <button className="underline font-bold" onClick={() => q.refetch()}>
              Retry
            </button>
        ...
        {!q.isLoading && !q.isError && (q.data ?? []).length === 0 && (
          <p className="p-3 text-sm font-medium font-display text-muted-foreground">
            No users found.
          </p>
        )}
      </div>
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent align="end" className="w-48 sketch-card wobbly-sm font-sans">
        ...
        <DropdownMenuItem
          onClick={onWipe}
          className="text-destructive font-bold cursor-pointer hover:bg-destructive hover:text-white"
        >
