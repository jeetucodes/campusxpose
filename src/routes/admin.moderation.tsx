...
function Moderation() {
  ...
  const evidence = useQuery({
    ...
    enabled: !!token,
    queryFn: () => listEv({ data: { token: token! } }),
  });
      ...
      <section>
        ...
        {evidence.isError && (
          <p className="text-sm text-destructive">
            Failed to load evidence: {(evidence.error as any)?.message ?? "Unknown error"}
          </p>
        )}
              ...
              <div className="relative">
                {e.type === "image" ? (
                  ...
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="grid h-32 place-items-center rounded-lg bg-surface-2 text-3xl"
                  >
                    📄
                  </a>
                )}
                {e.is_verified && (
                  <span className="absolute left-1 top-1 rounded-full border border-success/40 bg-success/20 px-2 py-0.5 text-[10px] font-semibold text-success shadow">
                    ✓ Verified
                  </span>
                )}
              </div>
              <div className="mt-2 flex gap-1">
                {e.is_verified ? (
                  <Button
                    ...
                    onClick={() => setVerified(e.id, false)}
                  >
                    {busy === e.id ? "…" : "Unverify"}
                  </Button>
                ) : (
